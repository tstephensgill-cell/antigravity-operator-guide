...
17. Advanced Worker/Subagent Gotchas: Native subagents are hidden background worker chats created with tools such as `define_subagent`, `invoke_subagent`, and sometimes `send_message`. Definitions are session-bound; local worker markdown remains source of truth. Review tool flags such as `enable_write_tools`, `enable_subagent_tools`, and `enable_mcp_tools`. Pure-logic workers may need `BOOTSTRAP_DELEGATED_BY_DISPATCHER` from the primary Anti session rather than reading BOOTSTRAP.md themselves. Strict JSON workers must output only JSON, with no `BOOTSTRAP_LOADED` wrapper text or markdown fences. Transcript proof should include conversation IDs and tool-use evidence such as `ExecutedTools` where available. Do not invent transcript paths; require verified local evidence. Keep swarms small; high concurrency can cause 429s, while concurrency around 4 has been safer. Stop on rate limits instead of retrying blindly. Partial worker fixes are not full passes if returned fields still violate the worker contract. External Platform Mode Caution: Platform UI modes may exist outside Anti OS governance files. Treat user-provided platform-mode details as platform context, not Anti source-of-truth. Do not claim such modes are Anti governance unless verified from Anti files. If a platform mode appears to reduce manual review or safety prompts, preserve Anti OS governance strictly: bootstrap first, explicit stage authority, numbered approval menus, exact option scope, no auto-advance, no hidden helper steps, and proof before completion.
